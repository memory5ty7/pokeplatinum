#include "macros/btlcmd.inc"

    .data

_000:
    CompareMonDataToValue OPCODE_EQU, BTLSCR_MSG_TEMP, BATTLEMON_CUR_HP, 0, _031
    //CompareMonDataToValue OPCODE_EQU, BTLSCR_MSG_TEMP, BATTLEMON_HELD_ITEM, ITEM_EJECT_PACK, _after_anim
    PlayBattleAnimation BTLSCR_MSG_TEMP, BATTLE_ANIMATION_HELD_ITEM
    Wait 

_after_anim:
    CompareMonDataToValue OPCODE_EQU, BTLSCR_MSG_TEMP, BATTLEMON_HELD_ITEM, ITEM_RED_CARD, _029
    // TODO: Eject Button text
    RemoveItem BTLSCR_MSG_TEMP
    UpdateVar OPCODE_FLAG_ON, BTLVAR_BATTLE_CTX_STATUS, SYSCTL_HIT_DURING_SHADOW_FORCE
    Call BATTLE_SUBSCRIPT_ATTACK_THEN_SWITCH_OUT
    UpdateVar OPCODE_FLAG_OFF, BTLVAR_BATTLE_CTX_STATUS, SYSCTL_HIT_DURING_SHADOW_FORCE
    GoTo _031

_029:
    PrintMessage pl_msg_00000368_01275, TAG_NICKNAME_NICKNAME_ITEM, BTLSCR_DEFENDER, BTLSCR_ATTACKER, BTLSCR_ATTACKER
    Wait 
    RemoveItem BTLSCR_MSG_TEMP
    Call BATTLE_SUBSCRIPT_FORCE_TARGET_TO_SWITCH_OR_FLEE

_031:
    End 